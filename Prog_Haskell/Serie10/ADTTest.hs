...
{-
delete :: Int -> Set a -> Set a
delete y (Set xs) = if y == xs then xs else delete x xs
-}
intersect :: Set a -> Set a -> Set a
intersect (x:xs) (y:ys) = if member x (y:ys) then x:intersect (y:ys) xs else intersect (y:ys) xs 
